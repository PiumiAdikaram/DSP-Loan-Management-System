...
<table class="masterlist">
    ...
    $sql="SELECT* FROM cashcollector";
    $result=$conn->query($sql);

    if($result->num_rows>0){
        while($row=$result->fetch_assoc()){
            echo "<tr><td>".$row["cc_Id"]."</td><td>".$row["userName"]."</td><td>".$row["email"]."</td><td>"
            ."<a href='bremovecc.php?cc_Id=$row[cc_Id]'>Remove</a>"."</td>";
        }
        echo "</table>";
    }
        else{
